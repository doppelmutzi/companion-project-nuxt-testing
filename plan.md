...
- [ ] **2. Server API route + `useFetch`** — Create `server/api/todos.get.ts` with an in-memory todo array. Refactor `app/pages/index.vue` to seed the Pinia store from `useFetch('/api/todos')` instead of hardcoding todos in the store.
- [ ] **3. Dynamic todo detail page** — Add `app/pages/todos/[id].vue` using `useRoute()` to read the param, display todo details, and throw `createError({ statusCode: 404 })` if not found. Add `useHead` to set a dynamic `<title>`.
...
- [ ] **7. Delete `app/views/Todos.vue`** — Remove the unused duplicate of `index.vue`.
- [ ] **8. Update `.github/copilot-instructions.md`** — Reflect all new Nuxt features, file structure, and data flow changes.